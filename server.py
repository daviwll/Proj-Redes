...
class ServerGUI:
    def __init__(self, master):
        self.pending_downloads = {}   # addr -> nome do arquivo
        self.master = master
        ...
        if conn:
            try:
                ...
                if command.startswith("get "):
                    ...
                    self.pending_downloads[client_addr] = filename
                elif command == "exit":
    ...
    def handle_client(self, conn, addr):
        ...
        try:
            ...
            while self.running:
                if addr in self.pending_downloads:
                    nome = self.pending_downloads[addr]
                    caminho_completo = os.path.join(os.getcwd(), os.path.basename(nome))  # Garante salvar no diretório atual

                    # Recebe o tamanho do arquivo
                    header = conn.recv(1024).decode(errors="ignore").strip()
                    if not header.startswith("[INFO]"):
                        self.message_queue.put(f"[!] Cabeçalho inválido de {addr}: {header}")
                        del self.pending_downloads[addr]
                        continue

                    try:
                        tam = int(header.split()[1])
                    except:
                        self.message_queue.put(f"[!] Tamanho inválido de {addr}")
                        del self.pending_downloads[addr]
                        continue

                    conn.send(b"READY")  # Confirmação
                    ...
                    try:
                        with open(caminho_completo, "wb") as f:
                            restante = tam
                            while restante > 0:
                                chunk = conn.recv(min(4096, restante))
                                if not chunk:
                                    break
                                f.write(chunk)
                                restante -= len(chunk)
                            if restante == 0:
                                self.message_queue.put(f"[✓] Arquivo '{nome}' salvo em {caminho_completo}")
                            else:
                                self.message_queue.put(f"[!] Arquivo incompleto de {addr}")
                    except Exception as e:
                        self.message_queue.put(f"[!] Erro ao salvar: {e}")

                    del self.pending_downloads[addr]
                    continue

                # 2) Mensagem normal (texto)
                data = conn.recv(4096)
                if not data:
                    break
