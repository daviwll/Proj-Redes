...
class ServerGUI:
    ...
    def create_widgets(self):
        ...
        # Botões de encriptação e descriptação
        self.encrypt_btn = ttk.Button(self.command_frame, text="Encriptar", 
                                    command=lambda: self.send_command("encrypt"))
        self.decrypt_btn = ttk.Button(self.command_frame, text="Descriptografar", 
                                    command=lambda: self.send_command("decrypt"))
        self.backup_btn = ttk.Button(self.command_frame, text="Backup", 
                                   command=lambda: self.send_command("backup"))
    ...
    def setup_layout(self):
        ...
        self.encrypt_btn.grid(row=0, column=3, padx=5, pady=5)
        self.decrypt_btn.grid(row=0, column=4, padx=5, pady=5)
        self.backup_btn.grid(row=0, column=5, padx=5, pady=5)
    ...
    def send_command(self, command=None):
        ...
        # Se o comando vier do botão, usa ele, senão pega do entry
        if command is None:
        ...
        if conn:
            try:
                # Envia o comando em minúsculas para garantir consistência
        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    self.message_queue.put(f"[!] Erro ao receber resposta de {addr[0]}:{addr[1]}: {e}")
                    break
